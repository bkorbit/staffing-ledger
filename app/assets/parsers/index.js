...
import { parseCSV } from './csv.js?v=259a4aa';
import { googleAdsDaily, googleAdsChanges } from './google-ads.js?v=259a4aa';
import { metaDaily, metaActivity } from './meta-ads.js?v=259a4aa';
import { dspDaily } from './dsp.js?v=259a4aa';
import { redditDaily, linkedinDaily } from './social.js?v=259a4aa';
import { cm360Daily } from './cm360.js?v=259a4aa';
...
export const PLATFORMS = {
  ...
  dv360:      { label: 'Display & Video 360', group: 'Programmatic', files: true, parser: 'dsp',
                hint: 'Reports → Offline reporting → Create report, type Standard. Dimensions: Date, Campaign, Insertion Order, Line Item, Creative. Metrics: Impressions, Media Cost (Advertiser Currency) or Revenue (Adv Currency). Time: daily, the flight to date. Download as CSV.' },
  ttd:        { label: 'The Trade Desk', group: 'Programmatic', files: true, parser: 'dsp',
                hint: 'Reports → My Reports → Create report. Dimensions: Date, Campaign, Ad Group, Creative. Metrics: Impressions, Advertiser Cost (USD). Granularity: daily, the flight to date. Export as CSV, not XLSX.' },
  viant:      { label: 'Viant (Adelphic)', group: 'Programmatic', files: true, parser: 'dsp',
                hint: 'Reporting → Campaign report by day. Columns: Date, Campaign (or Order), Ad Group or Line Item, Creative, Impressions, Advertiser Spend. CSV.' },
  amazon_dsp: { label: 'Amazon DSP', group: 'Programmatic', files: true, parser: 'dsp',
                hint: 'Reports → Create report → Campaign report, Daily. Columns: Date, Order, Line item, Creative, Impressions, Total cost. Amazon calls a campaign an Order. CSV.' },
  vistar:     { label: 'Vistar Media (DOOH)', group: 'Programmatic', files: true, parser: 'dsp',
                hint: 'Reporting → Campaign report by day. Columns: Date, Campaign, Insertion Order or Line Item, Creative, DMA, Impressions, Spend. CSV. DMA becomes markets.' },
  dsp:        { label: 'DSP (older uploads: DV360 / TTD)', group: 'Programmatic', files: false, parser: 'dsp', hint: 'recorded before the platforms were split (112) — pick DV360 or The Trade Desk for new uploads' },
