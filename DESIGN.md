...
description: The Ledger Console — a flat, brand-locked financial operations shell where Montserrat speaks and DM Mono counts.
colors:
...
- Montserrat for words, DM Mono for numbers — never swapped, never mixed within one element.
...
### Primary
- **Deep Ledger Green** (`#0a493c`): The system's authority color. Page headings (`h1.page`), the sidebar brand mark, table totals rows, positive figures, and the forecast chart's revenue series. If a number or heading needs to read as "this is the real, settled truth," it's this green.

### Secondary
- **Muted Ledger Green** (`#48a278`): The active/positive/hover register — primary-button hover, "matched" and "ok" status text, links inside summary notes, the forecast chart's overhead series, and (as a near-duplicate custom property in the codebase today) the good-pill border and input focus ring. One color, several call sites; worth consolidating the underlying `--brand-2`/`--brand-3` custom properties next time that file is touched.
...
### Tertiary
- **Signal Gold** (`#fabf4d`): The single most important color in the system. Reserved for the forecast chart's net-profit line — nothing else. The comment at its point of use in the codebase says it plainly: "Net wears accent gold: the most important line." Do not spend this color on anything else, including the warn-pill border it currently also happens to style (see Do's and Don'ts).
...
**Label/Data Font:** DM Mono (with `monospace` fallback) — reserved entirely for anything that is a number, a label, a badge, or a column header.
...
### Hierarchy
- **Headline** (700, 20px, 1.2 line-height): Page titles (`h1.page`), always in Deep Ledger Green.
- **Title** (700, 14.5px, 1.2 line-height): Client names, modal titles, brand-name mark — section-level identifiers.
...
- **Label** (500, 9px, uppercase, 0.12em tracking, DM Mono): Eyebrows, summary-strip labels, table headers, mono form labels — the small-caps mono vocabulary that appears on almost every screen.
- **Data** (400, 11px, DM Mono, tabular numerals): Every table cell, badge, and numeric input. `font-variant-numeric: tabular-nums` is set wherever figures line up in a column, so digits never jitter as they change.

### Scale

The five roles above are anchor points on a continuous, fine-grained scale, not the whole ramp — the interface runs a real 30-step scale from 9px up to 22px because a dense financial console has that many distinct jobs for text to do (a table header is not a table cell; a KPI value is not a page headline). Two tracks exist side by side:

- **Chrome scale (fixed px):** `9px` (micro-label: eyebrows, mono labels, table headers, kbadges) → `10px` (index text) → `10.5px` (dense grid/sum/fc table cells) → `11px` (default table cell, summary note) → `11.5px` (button label, emphasized client row) → `12px` (primary-button label, hint text) → `12.5px` (body text, input values) → `13px` (nav item) → `13.5px` (row/deal name) → `14.5px` (client-card title) → `15px` (modal title) → `17px` (sidebar brand mark) → `18px` (KPI value) → `20px` (page headline) → `22px` (login heading). These sizes are absolute; they do not respond to the fluid root below.
- **Editor micro-scale (fluid rem):** `0.56rem`–`0.82rem` in twelve steps, used only inside the deal-editor's line/card controls (`.pf`, `.kbadge`, `.lgp`, `.cgp`, `.colhead`, `.totalsgrid`, `.cardnote`, `.pmcell`, and siblings). Because these are `rem`, they scale with the fluid `html { font-size: clamp(13px, 0.55vw + 11.2px, 16px) }` root — the one part of the UI that actually gets denser on small screens and roomier on large ones.
- The root itself is documented at its two fixed endpoints, `13px` and `16px`.

### Named Rules
**The Two-Voice Rule.** Montserrat speaks; DM Mono counts. A heading, a sentence, a button label — Montserrat. A number, a table cell, a badge, a column header — DM Mono. Never swap them, never mix them within one element.
...
### Buttons
- **Shape:** 4px radius, DM Mono label at 11.5px.
...
### Data Tables
- **Style:** collapsed borders, raised-paper background, hairline cell borders, right-aligned numeric columns (first column stays left-aligned), DM Mono at 10.5–11px with tabular numerals so figures never jitter.
- **Header:** uppercase DM Mono label at 9px, light gray background, 2px Deep-Ledger-Green bottom border on the most important table variant (`.acct`); sortable headers show a small directional indicator in brand green.
...
### Status Pills
- **Style:** full-round (`99px`), DM Mono at 10px, 2px/8px padding, hairline border by default.
- **Good:** Mint Tint background, Deep Ledger Green text and border.
- **Warn:** Alert Amber Tint background, Alert Amber text, Signal Gold border (see Do's and Don'ts — this is the one place Gold currently escapes its one-line rule).
- **Bad:** Alert Rust Tint background, Alert Rust text and border.

### Inputs / Fields
- **Style:** 34px height (28–30px in dense in-table contexts), hairline border, 4px radius, DM Mono value text, raised-paper background.
...
### Navigation
- **Style:** 184px fixed sidebar, raised-paper background, hairline right border, sticky with independent scroll. Section labels are uppercase DM Mono at 9px. Nav items are 13px medium-weight Montserrat.
...
- **Do** write every heading and sentence in Montserrat, and every number, label, badge, and column header in DM Mono — never swap them.
...
- **Don't** spend Signal Gold on the warn-pill border just because it's already wired that way today; that's palette drift worth fixing, not a second sanctioned use.
