...
**Deferred, not forgotten: a KPI ribbon.** A headline-numbers strip above the chart (net, and likely revenue/GP alongside it) is worth building on `forecast.html` too, but as its own considered piece — not a single oversized number bolted onto the chart. (Sales already has one, reusing the shared `.summary-strip`/`.summary-cell` component from Cashflow.) Build it as a separate task when asked.
