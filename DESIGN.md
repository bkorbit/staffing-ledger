...
### Named Rules
**The One Gold Rule.** Within the forecast chart, Signal Gold appears in exactly one place: the net line. It is not a general accent, a warning color, or a highlight there — its rarity is what makes it legible as "the number that matters." It also appears as a confidence-tier step in the sales pipeline chart (a separate, confirmed, pre-existing exception — see Sales pipeline confidence ramp) and as the PTO segment in Team Hours' utilization bar (a second confirmed exception — see Utilization Mini-Bar's Priority composition fill; Boris's own call, since it's the only yellow/gold in the closed palette) — three different jobs, three different charts. Don't add a fourth without going through the same conversation.
