...
### Hierarchy
- **Title** (700, 14.5px, 1.2 line-height): Client names, modal titles, brand-name mark — section-level identifiers. Team, Client Profitability, and Project Hours additionally lead with a page-title heading (700, 22px, Deep Ledger Green) above their content — a denser, richer page benefits from the "where am I" reinforcement a bare nav highlight doesn't give; other pages still rely on the sidebar alone and aren't required to add one. Team Hours deliberately dropped its own heading — Boris's call, so the range selector reads as the first thing on the page rather than a heading sitting above it doing the same "where am I" job the sidebar's current-nav highlight already does.
...
- **Floating vs. carded tables:** most `table.ledger`/`table.acct` instances sit inside an `.sc-panel` card (hairline border, white background) — Cashflow, Client Profitability, Project Hours. Team Hours is a deliberate exception: its table sits directly on the page background with no wrapping card, matching the KPI ribbon and range selector next to it, which never had one — Boris's call, so the whole page reads as one continuous surface instead of the table looking like a separate boxed-off region. Don't silently re-add the card there; ask first if it looks like it's missing.
