...
### Chart categorical ramp (department series — the brand green, stepped)
Six steps of the brand's own green for the one chart that stacks many equal categories — Project Hours' hours-per-week-by-department — and nothing else: `--cat-1` `#48a278` Muted Ledger Green (leads), `--cat-2` `#0a493c` Deep Ledger Green, `--cat-3` `#19a789` Ledger Teal, `--cat-4` `#2e6e56`, `--cat-5` `#3bc9ac` Logo Teal, `--cat-6` `#57cb96`; `--cat-other` `#b5b5b5` for the fold. Boris supplied the extended kit (`#0a493c #2e6e56 #48a278 #19a789 #3bc9ac #57cb96 #dcf4e9`) on 8 Sep 2026 and chose it over a seven-hue alternative: brand purity first. The **order is the mechanism**: because it is one hue stepped by lightness, and lightness survives every colour-vision deficiency, the dark/light alternation above clears ΔE≥15 (normal) and ≥16 (protan/deutan/tritan) on every adjacent pair — the natural dark→light ramp does NOT, failing at three adjacent steps, so never re-sort these by lightness. The kit's mint `#dcf4e9` is excluded: 1.13:1 on white, a segment in it reads as a gap. The three light steps sit under 3:1 on white and rely on the 1px segment gaps, legend and tooltip as relief. Known trade, accepted: a single-hue ramp reads as a scale, so identity comes from the legend, not the colour alone. Six is the ceiling; a seventh department folds to Other. Colour follows the department name company-wide (headcount order, or an explicit pin), never a project's own ranking.

### Named Rules
**The One Gold Rule.** Within the forecast chart, Signal Gold appears in exactly one place: the net line. It is not a general accent, a warning color, or a highlight there — its rarity is what makes it legible as "the number that matters." It also appears as a confidence-tier step in the sales pipeline chart (a confirmed, pre-existing exception — see Sales pipeline confidence ramp), as the PTO segment in Team Hours' utilization bar (a second confirmed exception — see Utilization Mini-Bar's Priority composition fill; Boris's own call, since it's the only yellow/gold in the closed palette), and as the 90–100% band of Team Hours' logged/capacity percentage (`.cap-flag.high`, a third confirmed exception, same reasoning — the one color left between plain ink and Alert Rust once Alert Amber is already the under-capacity band). That one started as gold *text* and was audited out — 1.66:1 against white, far under WCAG's 4.5:1 — so it's gold as a light chip background with ink text instead, the same fix applied to the under-capacity band's amber text (3.72:1, also failing). Four different jobs, four different places. Don't add a fifth without going through the same conversation.
...
### Hierarchy
- **Title** (700, 14.5px, 1.2 line-height): Client names, modal titles, brand-name mark — section-level identifiers. No page leads with its own page-title heading (700, 22px, Deep Ledger Green) any more — Team, Client Profitability, and Project Hours each tried one and dropped it, Boris's call each time: the sidebar's current-nav highlight already answers "where am I," and a heading above the page's own first control (a filter row, a range selector) was doing that job twice. The 22px/Deep-Ledger-Green title style stays defined above for any future case that earns it back, but the default for every page is now no heading at all.
...
- **Priority composition fill** (Team Hours' utilization column only): internal (Deep Ledger Green) and time-off (Signal Gold — a confirmed second exception to the One Gold Rule, Boris's call, the only yellow/gold in the closed palette) reserve their own room against capacity FIRST; client (Muted Ledger Green) gets whatever's left, and only the client segment can turn Alert Rust for the portion beyond that reduced budget — someone loaded with internal meetings or PTO goes red on client hours sooner than a flat total-vs-capacity check would show, since their non-client time already spent part of the budget. Internal and PTO never turn red themselves, no matter how far over someone is. Scaled to whichever is bigger, capacity or actual hours worked, so the bar always fills its track — under capacity leaves real empty gray track (unused capacity), at/over capacity fills the whole bar with no empty remainder. A missing comp period is no longer a zero here: `hours_page()` (038) defaults an unset weekly_capacity to 40 server-side, since Team is the one place capacity gets set and an incomplete record shouldn't silently read as "no capacity" everywhere downstream. An explicit 0 (someone actually entered a 0-hour/week comp period) still reads as every client hour being "over" — the same honest red signal as genuine overwork, just scoped to a real entered zero rather than an absent record. Team Hours' pacing column stays on the older two-tone, capped-at-100%-with-a-text-overage-note rule below; it isn't part of this priority system.
- **Caption:** a small mono line below the bar spelling out the breakdown in words (e.g. "38h client · 4.5h internal · 2h PTO / 60h cap") rather than adding more bar segments than the fill already has — text carries any further detail; the bar stays a shape you can scan at a glance. On Team Hours, all three bar columns reserve two lines' worth of caption height regardless of actual wrap (`.barcap`) so their bars align to the same top edge even though utilization's caption is usually the only one that wraps.
- **Delta note:** a small mono caption ("+3.2h over capacity") appearing only when there's real overage; otherwise the caption line alone stands as the reading. A denominator of zero (nothing assigned yet, nothing to compare) shows an explicit "not assigned yet" placeholder in Slate rather than a bar reading 0% — that's real information, not a gap to hide. (The priority composition fill above is the one exception: its own zero/missing-capacity case renders red, not a placeholder, per that section.)
...
- **Expanded row, two shapes.** A row that opens has two legitimate forms, and which one is right depends on whether the detail shares the parent's measures. Team Hours' **`.expand-row`** pops a free-form panel out under the row (`colspan`, its own two-column grid) — right when the detail is a different kind of content, with no column to sit in: bars, a time-allocation split. Client Profitability's **`.subrow`** keeps the children in the parent table as real rows — right when the detail is the same record one level down on the *same* columns (a client's deals, each with the same revenue/COGS/GP/labor/rate figures). The payoff is alignment: every child figure lands under the header it belongs to, and the parent row reads as the visible sum of the rows beneath it, which a nested table with its own column widths can't deliver. Don't reach for a nested `<table>` inside an `.expand-row` for this case — that shape was considered and rejected for exactly that reason. A `.subrow` block sits on Table Header Wash like `.expand-row` does, indented 28px, and may open with a `.subnote` context line in the small mono label vocabulary. It used to carry a Muted Ledger Green rail down its left edge; Boris removed it on 8 Sep 2026 ("it doesn't make any sense") — the indent and the wash already say "inside the client", and the rail was a second voice for the same thing. The unclaimed remainder row lost its dashed top rule at the same time: it is a muted deal row whose sub-line says what it is. A child that is itself flagged takes `.risk`, the Alert Rust Tint the Risk row above gives a parent. **Every selector in the block is prefixed `table.ledger`** — the bare `tr.subrow td` form ties with `table.ledger td` on specificity and loses on source order, so the padding and borders would silently never apply.
- **Expanded row, third shape — the split (Project Hours).** An opened project is `.expand-row` underneath (its detail is people and two charts, not the parent's record on the parent's columns), laid out as `.expand-split`: a 2:3 grid, the **people table** (person · assigned-vs-logged bar · assigned · logged — nothing a person can't honestly fill, so no dash-filled client/date cells) on the left and the **two reports stacked** on the right (hours per ISO week by department over the whole flight with the page's range shaded; revenue and gross profit, actual vs plan, per month). Boris iterated this by hand from a 1:1 stack to 1:2 to 2:3, and to the panel edges lining up: the people panel stretches to the chart stack's bottom edge, its rows keep their natural height at the top, and its totals bar is pinned to the bottom at `.chart-legend`'s own height so the three footers read as one line. The charts carry no explanatory sub-captions — an eyebrow and a one-line stat (avg/peak hours; to-date actual vs plan) is all; the tooltip carries the rest. Series colors on the weekly chart are the closed brand palette in a fixed order keyed by department NAME company-wide (never by a project's own ranking, so a department is the same color on every project), and a sixth department folds into "Other" rather than inventing a hue. On the revenue chart two hues carry the measure (revenue Deep Ledger Green, GP Ledger Teal) and line style carries plan vs actual (plan dashed and lighter, per the measured-vs-forecast rule); the actual line simply ends at the last fully-closed month. Note the model's own consequence: for search/social/retainer lines planned GP sits exactly on planned revenue (gp == billable); only programmatic and real COGS separate them.
- **Client Profitability closes an open client with the same two reports** (`.client-charts`, 8 Sep 2026): under the deal rows and the remainder, full width, side by side at a 940×220 frame, summed across the client's deals by `client_detail()` (db/089) and drawn by `assets/detail-charts.js` — the one copy both pages use. They match the client row above them to the cent: the unclaimed remainder is out of the charts for the reason it is out of the total. The two panels share one height whatever their legends wrap to (legend pinned to the bottom, chart centred above it). The hours axis ends at the current week on both pages — nothing can be logged in the future, so the rest of the flight is not drawn as empty track; the revenue axis keeps the whole flight because its plan does extend. On the client the hours are stacked **by project**, not department — Boris's revision after the first real look ("which project is eating the hours" is the client-level question; Project Hours still stacks its one project by department); series are ordered by flight start so a project keeps its colour as hours shift. A closed month with nothing invoiced is drawn as $0, not as a break in the actual line (090). The month axis keeps 28px of right padding so the last label is not clipped. **The window is where the information is** (Boris, after the second real look): both charts trim leading and trailing empty stretches, and a $0 month or week is drawn only between two that have something — the gap is information, a year of nothing before the first invoice is not. The hours chart also trims a dribble at either end (a bucket under 1% of the tallest, e.g. a stray quarter-hour logged a year early) so one mislogged entry cannot stretch the axis, and once its window is longer than four months the bars become **months** (a week is bucketed by the month its Monday falls in) — 26+ weekly bars read as noise where six monthly ones read as a trend; the legend bar says which grain is drawn. The client chart carries a **Proj / Dept** switch on the card head (`.rp.rp-sm`, the quick-range pill at 22px, beside the stat line — Boris found the legend-bar version unintuitive to click; remembered per browser); Project Hours has no toggle because one project only stacks by department. Deal rows on Client Profitability carry no sub-line either (the QB project name is the cell's hover title, the `no QB project` pill sits inline) and no measured · forecast split under revenue (hover title) — the same single-line discipline as Project Hours' table. The fold past six series is never a bare "Other": it is named by what it holds ("+3 smaller projects") and its tooltip itemises each member; by project the six largest by hours take the colours, and a department the staff list does not know still gets a colour while slots remain.
