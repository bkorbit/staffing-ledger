-- ============================================================================
--  034 — a per-STAFF assigned-hours total, for Team Hours' new pacing columns.
--
--  hours_page() already aggregates assignments per deal (deal_planned), but
--  "how much of this person's capacity is even planned" and "are they on
--  pace against their own assigned client hours" both need the sum PER
--  PERSON instead. assignments is still empty (no entry mechanism yet) — this
--  returns [] until that changes, same as deal_planned already does.
-- ============================================================================
...
comment on function hours_page is
  ...
  'person''s total assigned hours across every deal in the window — empty '
  'until assignments has a real entry mechanism.';
