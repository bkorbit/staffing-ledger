-- ============================================================================
--  017 — programmatic COGS on real terms: month-end + N days, configurable.
--
--  Forecast COGS from shaped deals was leaving mid spend month — an assumption.
--  The business pays media on terms: due N days after the LAST day of the spend
--  month (October spend -> October 31 + 45 days -> December 15). N lives in
--  settings under programmatic_cogs_due_days, default 45, editable on the
--  Settings page; the function reads it on every call.
--
--  Cash consequence, worth knowing: client collections land around month-end +30
--  on the observed curves, so at +45 the programmatic book mostly collects BEFORE
--  it pays — the mid-month version overstated the trough.
-- ============================================================================

insert into settings (key, value, set_by)
values ('programmatic_cogs_due_days', '45', 'migration-017')
...
comment on function cashflow_forecast is
  'Half-month cash position in three bands. Forecast COGS from shaped deals goes '
  'out at spend-month-end plus programmatic_cogs_due_days (settings, default 45).';
