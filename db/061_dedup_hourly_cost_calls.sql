-- ============================================================================
--  061 — hours_page: resolve staff_hourly_cost() once per distinct
--  (staff_id, worked_on), not once per raw time_entries row.
--
--  057 cut staff_annual_burdened_cost's buffer hits from 31,640 to 11,288 for
--  the same range (batching its settings/rate lookups), but EXPLAIN ANALYZE
--  on hours_page() still showed ~2.5s — buffer hits went down, wall-clock
--  didn't move. That means the cost was never disk/buffer I/O; it's the
--  sheer number of times staff_hourly_cost()/staff_annual_burdened_cost()
--  get invoked as scalar functions. Each call carries fixed overhead
--  (parse/plan/executor setup for a multi-CTE function body) regardless of
--  how cheap its internals are, and the labor CTE called it once per RAW
--  time_entries row — someone logging hours against three deals in one day
--  recomputed the exact same day's rate three times.
--
--  Pre-resolving the rate once per distinct (staff_id, worked_on) pair and
--  joining it back is a pure dedup: same function, same two inputs, same
--  output for every row that shares them — just not recomputed redundantly.
--  Nothing else in hours_page changes.
-- ============================================================================
...
comment on function hours_page is
  ...
  'capacity gets a safe default, never pay. day_rates (061) resolves '
  'staff_hourly_cost() once per distinct (staff_id, worked_on) instead of '
  'once per raw time_entries row — someone logging hours against several '
  'deals in one day no longer recomputes that day''s rate multiple times.';
